...
zmqWrapper::zmqWrapper(const char* _sockType, const std::string _ip, const int _hwm = 1, const int _linger = 25, 
	...
	if( strcmp("publish", _sockType) == 0 ) {
		sockType = publish;
		c = &zmqWrapper::sockPublish;
	}
	else if( strcmp("subscribe", _sockType) == 0 ) {
		sockType = subscribe;
		c = &zmqWrapper::sockSubscribe;
	}
	else if( strcmp("request", _sockType) == 0 ) {
		sockType = request;
		c = &zmqWrapper::sockRequest;
	}
	else if( strcmp("reply", _sockType) == 0 ) {
		sockType = reply;
